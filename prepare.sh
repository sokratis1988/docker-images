...
function get_from_pypi() {
  ...
  if [[ $ERC == true ]]; then
      JQ_FILTER+=' | map(select(test("(?:[.-]?rc[0-9.]*$)") | not))'
  fi

  if [[ $EAC == true ]]; then
      JQ_FILTER+=' | map(select(test("(?:[.-]?(?:a|alpha|b|beta)[0-9.]*$)") | not))'
  fi
...
log_info "- alpha/beta           (--exclude-alpha)                : \e[33;1m${EAC:-false}\e[0m"
